...
class Dynamixel_XW430_T200_interface(Node):
    ...
    def hardware_loop(self):
        if self.latest_command is None or self.is_configuring: return
    ...
    def destroy_node(self):
        for sid in self.active_ids:
            try: self.packet_handler.write1ByteTxRx(self.port, sid, self.ADDR_TORQUE, 0)
            except: pass
        self.port.closePort()
        super().destroy_node()
